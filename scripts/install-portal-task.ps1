...
} elseif (-not (Test-Path -LiteralPath $portalExe)) {
    throw "Portal binary not found: $portalExe. Run install-portal-windows.ps1 first."
}
if ($ConnectLink) {
    $null = Get-PortalLoomUri $ConnectLink
} else {
    $ConnectLink = Get-PortalSavedValue $Root '.portal-connection.url'
    $null = Get-PortalLoomUri $ConnectLink
}
foreach ($required in @('portal.toml')) {
...
if ($BinaryPath -and $BinaryPath -ne $portalExe) {
    New-Item -ItemType Directory -Path (Split-Path $portalExe) -Force | Out-Null
    Copy-Item -LiteralPath $BinaryPath -Destination $portalExe -Force
}
