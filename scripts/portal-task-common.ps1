...
function Get-PortalLoomUri([string]$ConnectLink) {
    $uri = $null
    if (-not [Uri]::TryCreate($ConnectLink.Trim(), [UriKind]::Absolute, [ref]$uri) -or
        $uri.Scheme -notin @('http', 'https') -or
        [string]::IsNullOrWhiteSpace($uri.AbsolutePath.Trim('/')) -or
        $uri.Query -notmatch '(?:^\?|&)token=[^&#]+') {
        throw 'ConnectLink must be an HTTP or HTTPS Loom URL with a Being ID and a nonempty token.'
    }
    return $uri
}
