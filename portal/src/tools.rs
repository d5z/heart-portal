...
impl ToolHost {
    pub fn new(config: &PortalConfig) -> Self {
        let extension_manager = Arc::new(ExtensionManager::new(config.security.workspace_root.clone()));
        Self {
            config: config.clone(),
    ...
    /// Initialize extensions (called during startup)
        ...
        match name {
            ...
            _ => {
                ...
                if let Some(tool) = custom.get(name) {
                    ...
                    match self.extension_manager.call_tool(name, arguments).await {
                        Ok(result) => Ok(result),
                        Err(_) => Err(anyhow::anyhow!("Unknown tool: {}", name))
                    }
                }
            }
        }
    ...
    /// Reload extensions configuration and apply changes
    pub async fn reload_extensions(&self) -> Result<Vec<String>> {
        let changes = self.extension_manager.reload_config().await?;
        if !changes.is_empty() {
            self.needs_reconnect.store(true, std::sync::atomic::Ordering::SeqCst);
        }
        Ok(changes)
    }

    /// Get extension status
    pub async fn get_extension_status(&self) -> HashMap<String, (crate::extensions::ExtensionStatus, Option<String>)> {
        self.extension_manager.get_status().await
    }
